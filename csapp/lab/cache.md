...
需要在csim.c实现一个缓存模拟器.来操作traces目录下的文件.且不能有warning.
...
trace文件里面有三种操作,L,M,S操作

L对应loadData，就是加载一个数据，如果不在Cache中就miss.然后放进Cache（可能需要移除一个）

S对应storeData，这个操作可能引发的情况与L相同，也是相同操作，所以这俩个就是一样的

M对应modifyData，就是load+store，执行两次同一个函数就行了，但是因为load之后，数据肯定在Cache中，所以直接输出hit就行了

...
//此文件实现了Cool的语义检查.有三种passes:
//1.  这不是一个真正的pass,因为只有类被检查.
//    构造继承图并检查错误.有两个sub-passes:
//    检查类是否被重定义,并且继承只能从定义的类中
//    并检查继承图的cycles
//    如果在sub-passes里检测到错误,则停止编译.

//2.  为每个类构建Symbol tables.
//    这一步是独立完成的,因为方和属性具有全局的作用域.
//    所以,在进行类型检查之前,所有methods和attributes必须是已知的

//3.  继承图,已知为一个树结构,如果没有循环---将从root class Object重新开始遍历
//    对于每一个类,每个属性和方法都是typechecked的.
//    同时,检查标识符是否正确的定义/使用和使用多定义
//    在检查一个类之前先检查他的父类.
...
// Cool类型检查规则 需要 四个结构来typecheck一个 X class
// 这四项都封装在Environment里:
//      a. X中,从method names 到method definitions 的mapping
//      b. X中,从variable(local and attribute)到 definitions 的mapping
//      c. X以外的方法类,从method names和 class names到definitions的 mapping
//      d. the self class (X)
// 
//      c.使用class_table实现的,该表包含了所有类的从class names 到 InheritanceNodes
//      (包括从thereby到Environment)的 mapping
...
Environment::Environment(SymbolTable<Symbol, method_class> mt,
		  ...
		  self_class(sc)
{ 
  //为每一个method/variable tables push一个新的作用域
  //这样新的method/variable 就不会与先有的的冲突
  method_table.enterscope();
  var_table.enterscope();
}

//
// 大部分 "new" environments是父类的environment替换self class
// 一个很小但很重要的一点是复制了method table和var table的结构
// 这样的话在新环境添加原始环境就不会产生影响
//
...
//semant_error是一个重载的函数,用于语义分析期间的报错阶段,有三个版本:
// 1.ostream& ClassTable::semant_error()
// 2.ostream& ClassTable::semant_error(Class_ c)
//   打印class c的行数和文件名
// 3.ostream& ClassTable::semant_error(Symbol filename, tree_node *t)  
//   打印树节点*t提取的的行数和文件名   
...
// inheritance graph
// 以下的函数允许创建 inheritance graph的节点,并从节点设置/检索父节点
...
// ClassTable
// ClassTable的构造函数 初始化了 从class names 到 inheritance graph nodes
// 的mapping,并且install了 basic classes.

// Cool 有五个基本类:
//  Object: root的层级,所以objects都是Objects
//  IO: 用于简单string和integer的input/output
//  Int: Integers
//  Bool: BOoleans
//  String: Strings

// User定义的类可以继承Object和IO,但是不能继承Int,Bool和String类.
...
//
// 一些特殊的类名在lookup table里installed,但没在class table
// 所以,这些类是存在的,但是不在继承层次结构的一部分
// No_class充当Object和其他特殊类的父类.
// SELF_TYPE 是一个 self class; 不能重新定义或者继承
// prim_slot i是一个code generator已知的类.
//
  addid(No_class,
...
// 
// The Object class 没有父类. 他的 methods如下:
//        cool_abort() : Object    退出程序.
//        type_name() : Str        返回一个代表class name的string
//        copy() : SELF_TYPE       返回a copy of the object
//
// 基本类不需要方法体--这些已经内置在运行时的系统中.
//
...
void ClassTable::install_class(InheritanceNodeP nd)
{
  ...
  // 类名合法则加入list of classes和symbol table中.
  nds = new List<InheritanceNode>(nd,nds);
  addid(name,nd);
}

//
// Install 所有用户定义的类,所有这些类都可以被继承,而不是基本类.
//
...
//
// 对于继承图的每一个节点,设置他们的父类,并把该节点加入父类的孩子list中
//
...
//
// 获取InheritanceNode并通过class table定位它和他的父节点
// 适当的添加父亲和孩子指针
//
...
//
// 这个method只能在mark_reachable执行之后跑
// 如果继承图中有任何不可方问的类,并且所有对check_improper_inheritance成功
// 则继承图有个cycle.
//
...
// 递归地将从参数中可访问的所有节点标记为可访问
// 初始化地用Object调用.
//
// 此函数被保证终止,因为从Object访问的继承层级结构的子图不能包含循环
// 如果通过check_improper_inheritance没有发现local errors的话.
//
...
//
// 下面的一组函数递归地遍历每个类的每个feature,
// 向类中有关feature的environment添加information 
// 这里catach了像 在类中重定义method/attribute names 的error
//
//////////////////////////////////////////////////////////////////////////
...
void ClassTable::build_feature_tables()
{
  ...
  root()->build_feature_tables();  // 递归的为root和所有后代构建feature_table
}
...
//
// 检查main类的存在,以及它是否可以访问继承层级中的Main方法
//
...
//
// 这些函数会比较复杂由于要处理SELF_TYPE还要注意避免为未定义的类生成多个错误信息
// (使用这些函数时已经报告了这些错误信息)
//
/////////////////////////////////////////////////////////////////////////
...
{
  // 如果其中一个类不存在,则返回true以减少虚假错误信息的数量.
  // 还为任何的't'提供 No_type<t
...
Symbol Environment::type_lub(Symbol type1, Symbol type2)
{
  ...
  // 3. If either is SELF_TYPE, convert it to the type of the class(将其转换为类的类型)
...
//
//  对于每个表达式的class,都有一个tc method来typecheck 它
//  tc method使用以前为每个类构造的环境
//  这个代码非常接近CollAId中类型推理规则的结构
///////////////////////////////////////////////////////////////////////////////
